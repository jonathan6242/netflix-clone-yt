...
import { loadProducts, timestampToDate } from "@/lib/stripe";
...
function Account({ products }: Props) {
  ...
  function getProductIdFromSubscription(
    ...
    const productId: string = product?._key?.path?.segments[6];
        ...
        <div className="flex flex-col gap-x-4 md:flex-row md:items-center">
          ...
          <div className="-ml-0.5 flex items-center gap-x-1.5">
            ...
            <p className="text-xs font-semibold text-[#555]">
              Member since {timestampToDate(subscription?.created?.toString()!)}
            </p>
          </div>
        </div>
          ...
          <div className="col-span-2 font-medium">
            {
              products.filter(
                (product) =>
                  product.id === getProductIdFromSubscription(subscription)
              )[0]?.name
            }
          </div>
